...
test('interrupted parse and index jobs recover to explicit failures and can be retried', t => {
  ...
  store.performJob(parse.task_id);
  ...
  store.performJob(index.task_id);
  assert.equal(store.query(kb, '关键词').results.length, 1);
});

test('deleting a queued document removes raw content, jobs and chunks without resurrection', t => {
  const store = ownedStore(t, directory(t));
  const {kb, id} = upload(store);
  const job = store.queue(kb, id, 'parse');
  store.deleteDocument(kb, id);
  store.performJob(job.task_id);
